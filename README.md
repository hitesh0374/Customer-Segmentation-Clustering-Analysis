...
DBSCAN – density-based, also flags outliers
...
ColumnDescriptionCustomerIDUnique identifier for each shopperGenderCustomer's genderAgeCustomer's ageAnnual Income (k$)Yearly incomeSpending Score (1-100)Score assigned by the mall based on purchase behavior
...
bashgit clone https://github.com/your-username/customer-segmentation.git
cd customer-segmentation
pip install numpy pandas matplotlib seaborn scipy scikit-learn jupyter

Then just open the notebook:

bashjupyter notebook customer_segmentation.ipynb

The dataset is fetched automatically from a public URL, so there's nothing extra to download.
